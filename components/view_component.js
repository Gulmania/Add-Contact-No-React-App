     ...
     return(<>
       <Add_Form  getData={getData} />
        ...
        <tr>
          ...
          <td>{item.email}</td>
          <td>{item.phone}</td>
          <td><button className="del_btn" onClick={()=>delHandler(item.id)}><i class="fa-solid fa-trash"></i></button></td>
          <td><button className="up_btn" onClick={()=>updateHandler(item.id)}><i class="fa-solid fa-file-pen"></i></button></td>
        </tr>
